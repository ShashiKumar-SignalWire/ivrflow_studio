(() => {
            ...
            return markFunction(function(seed, results, context, xml) {
              ...
              if (seed) {
                if (postFinder || preFilter) {
                  ...
                  i2 = matcherOut.length;
                  while (i2--) {
                    if ((elem = matcherOut[i2]) && (temp = postFinder ? indexOf4(seed, elem) : preMap[i2]) > -1) {
  ...
  var Session = class {
    ...
    allowsImmediateRender({ element: element2 }, resume) {
      const event = this.notifyApplicationBeforeRender(element2, resume);
  ...
  var SnapshotSubstitution = class {
    constructor(element2) {
      this.visitCachedSnapshot = ({ element: element3 }) => {
        var _a;
        const { id, clone } = this;
        (_a = element3.querySelector("#" + id)) === null || _a === void 0 ? void 0 : _a.replaceWith(clone);
      };
  ...
  var Action = class {
    constructor(element2, index, descriptor) {
      this.element = element2;
      this.index = index;
...
%o`, message, error2, detail);
  ...
  var Collapse = class extends BaseComponent {
    ...
    show() {
      ...
      let activeChildren = [];
      if (this._config.parent) {
        activeChildren = this._getFirstLevelChildren(SELECTOR_ACTIVES).filter((element2) => element2 !== this._element).map((element2) => Collapse.getOrCreateInstance(element2, {
          toggle: false
        }));
      }
    ...
    hide() {
      ...
      for (const trigger of this._triggerArray) {
        const element2 = getElementFromSelector(trigger);
  ...
  EventHandler.on(document, EVENT_CLICK_DATA_API$4, SELECTOR_DATA_TOGGLE$4, function(event) {
    ...
    for (const element2 of selectorElements) {
  ...
  var Tab = class extends BaseComponent {
    ...
    _keydown(event) {
      ...
      const nextActiveElement = getNextActiveElement(this._getChildren().filter((element2) => !isDisabled(element2)), event.target, isNext, true);
  ...
  // app/javascript/channels/message_room_channel.js
  var element = document.getElementById("conversation-room-id");
  console.log(element);
  if (element !== null) {
    $('[data-channel-subscribe="conversation"]').each(function(index, element2) {
      console.log(element2);
      var $element = $(element2);
      $element.animate({ scrollTop: $element.prop("scrollHeight") }, 1e3);
    });
    consumer_default.subscriptions.create({ channel: "MessageRoomChannel", message_room_id: element.getAttribute("data-conversation-id") }, {
      connected() {
